...
async function parseGLB( arybuf: ArrayBuffer ): Promise<[JSON, ArrayBuffer] | null> {
    //~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
    const dv        = new DataView( arybuf );

    if( dv.getUint32( GLB_MAGIC_BIDX, true ) != GLB_MAGIC ){    throw new Error( 'GLB magic number does not match.' );   }
    if( dv.getUint32( GLB_VERSION_BIDX, true ) != GLB_VER ){    throw new Error( 'Can only accept GLB of version 2.' );  }
    if( dv.getUint32( GLB_JSON_TYPE_BIDX, true ) != GLB_JSON ){ throw new Error( 'GLB Chunk 0 is not the type: JSON ');  }
    ...
    if( dv.getUint32( chk1_bidx + 4, true ) != GLB_BIN ){       throw new Error( 'GLB Chunk 1 is not the type: BIN ' ); }
    ...
    if( bin.byteLength != bin_len ){ throw new Error( 'GLB Bin length does not match value in header.' ); }
