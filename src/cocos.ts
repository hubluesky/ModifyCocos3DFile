...
export namespace cocos {
    ...
    export function loadAssetInDependent<T extends cc.Asset>(request: string, options?: cc.__private._cocos_asset_asset_manager_shared__IOptions): Promise<T> {
        return new Promise((resolve, reject) => cc.assetManager.loadRemote<T>(request, options, function (err, data) {
            if (err) reject(err);
            resolve(data);
        }));
    }
    ...
    export function deserialize(data: any): any {
        return cc.deserialize(data, null);
    }
}
